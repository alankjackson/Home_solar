...
foo16_a <- foo %>%  filter(Hour==16) %>% filter(Month=="Aug") 
foo16_b <- foo %>%  filter(Hour==04) %>% filter(Month=="Aug")

# Kernel density estimate
# Extend range of density estimate beyond data
# e = 0.1 * diff(range(foo16$Scaled_price))
e=0

dens = density(foo16_a$LZ_price, adjust=1, n=2056,
               from=min(foo16_a$LZ_price)-e, 
               to=max(foo16_a$LZ_price) +e)
dens = data.frame(x=dens$x, y=dens$y)
  
foo16_a %>% ggplot(aes(x=LZ_price))+
  geom_histogram(aes(y=..density..),# Histogram with density instead of count on y-axis
                   binwidth=50,
                   colour="black", fill="white") +
  geom_density(color="blue", size=2, alpha=0.3)+
  geom_line(data=dens, aes(x=x, y=y), color="red")+
  labs(title="August at 4PM - highly asymmetric distribution")

dens_b = density(foo16_b$LZ_price, adjust=1, n=2056,
               from=min(foo16_b$LZ_price)-e, 
               to=max(foo16_b$LZ_price) +e)
dens_b = data.frame(x=dens_b$x, y=dens_b$y)
  
foo16_b %>% ggplot(aes(x=LZ_price))+
  geom_histogram(aes(y=..density..),# Histogram with density instead of count on y-axis
                   bins=50,
                   colour="black", fill="white") +
  geom_density(color="blue", size=2)+
  geom_line(data=dens_b, aes(x=x, y=y), color="red")+
  labs(title="August at 4AM - highly symmetric distribution")
...
as_tibble(cbind(Price=Quartiles, Density=Emp)) %>% 
  ...
  geom_histogram(data=foo_sub, aes(y=after_stat(density), x=LZ_price), binwidth=50)+
  geom_line(color="red")

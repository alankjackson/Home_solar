...
foobar <- Solar_old %>% 
  head(50) %>% 
  # mutate(behind=lag(Solar_W)) %>% 
  mutate(behind=lag(Solar_W, default=0)) %>% 
  rowwise() %>% 
  mutate(one=min(Solar_W, behind)) %>% 
  mutate(two=abs(Solar_W-behind)) %>% 
  mutate(kWh=(0.25*(min(Solar_W, behind)) + 
             0.125*(abs(Solar_W-behind)))/1000) %>% 
  ungroup()
...
Smart <- readRDS(paste0(path, "Smart.rds")) %>% 
  filter(Date<mdy_hm("10-31-2022 00:00"))  

Tesla <- Solar_old %>% 
  filter(Timestamp>mdy_hm("1-1-2021 00:00")) %>% 
  filter(Timestamp<mdy_hm("10-31-2022 00:00"))  
